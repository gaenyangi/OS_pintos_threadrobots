...
void move_robot_to_req(struct robot* _robot) { // move toward the req payload
    ...
    if ((_robot->row < _robot->req_row )&& (draw_map_default[_robot->row+1][_robot->col]!='X')) {
        _robot->row++;// Move downwards
    }
    else if ((_robot->col < _robot->req_col) && (draw_map_default[_robot->row][_robot->col+1] != 'X')) {
        _robot->col++; // Move rightwards
    }
    else if ((_robot->row > _robot->req_row) && (draw_map_default[_robot->row-1][_robot->col] != 'X')) {
        _robot->row--; // Move upwards
    }
    else if ((_robot->row > _robot->req_col) && (draw_map_default[_robot->row][_robot->col-1] != 'X')) {
...
void move_robot_to_tar(struct robot* _robot) { // after get the payload, move to desitnation
    ...
    if ((_robot->row < _robot->tar_row) && (draw_map_default[_robot->row+1][_robot->col] != 'X')) {
        _robot->row++; // Move downwards
    }
    else if ((_robot->col < _robot->tar_col) && (draw_map_default[_robot->row][_robot->col+1] != 'X')) {
        _robot->col++; // Move rightwards
    }
    else if ((_robot->row > _robot->tar_row) && (draw_map_default[_robot->row-1][_robot->col] != 'X')) {
        _robot->row--; // Move upwards
    }
    else if ((_robot->row > _robot->tar_col) && (draw_map_default[_robot->row][_robot->col-1] != 'X')) {
        _robot->col--; // Move leftwards
    }
